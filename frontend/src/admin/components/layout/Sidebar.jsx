        ...
        <nav className="flex flex-1 flex-col gap-1">
          {navItems.map(({ to, label, icon }) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `flex items-center gap-3 rounded-xl transition-all duration-200 ${
                  isActive
                    ? 'bg-white font-bold text-slate-950 shadow-lg'
            ...
            >
              ...
              <span className={`font-body text-sm font-medium transition-all ${isCollapsed ? 'md:hidden' : 'inline'}`}>
                {label}
              </span>
            </NavLink>
          ))}
        ...
        <div className="mt-auto flex flex-col gap-1 pt-6 border-t border-white/5">
          ...
          {!isCollapsed && user && (
            <div className="px-4 py-3 mb-2 rounded-xl bg-white/5">
              <p className="text-xs font-bold text-white truncate">{displayName(user)}</p>
