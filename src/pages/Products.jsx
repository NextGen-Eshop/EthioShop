...
function ProductCard({ product, viewMode, catName, isAdmin }) {
    ...
    return (
      <motion.div variants={fadeInUp} layout>
        <Link
          ...
          className="group flex bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:shadow-indigo-100/40 hover:border-indigo-100 transition-all duration-300"
        >
          <div className="relative w-24 sm:w-56 shrink-0 overflow-hidden bg-gray-50">
            <motion.img
              src={product.image}
              alt={product.name}
            ...
            />
            {product.badge && (
              <span className="absolute top-3 left-3 px-2.5 py-1 bg-indigo-600 text-white text-[10px] font-bold uppercase tracking-wider rounded-full z-10">
                {product.badge}
              </span>
            )}
          </div>
          <div className="flex-1 p-5 flex flex-col justify-between">
            <div>
              ...
              <h3 className="text-base font-bold text-gray-900 group-hover:text-indigo-600 transition-colors mb-2 leading-snug">
                {product.name}
              </h3>
            ...
            <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
              <div className="flex items-baseline gap-2">
                ...
                {product.originalPrice && (
                  <>
                    <span className="text-sm text-gray-400 line-through">ETB {product.originalPrice.toLocaleString()}</span>
                    <span className="px-2 py-0.5 bg-green-100 text-green-700 text-[11px] font-bold rounded-full">-{discount}%</span>
                  </>
                )}
              </div>
              <div className="flex items-center gap-2">
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${
                  ...
                  product.stock <= 5 ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700'
                }`}>
                ...
                {isAdmin && (
                  <button
                    ...
                    className="p-2 bg-slate-900 text-white rounded-lg hover:bg-slate-800 transition-all shadow-md"
                    title="Admin Edit"
                  >
    ...
    <motion.div variants={fadeInUp} layout className="h-full flex">
      ...
      >
        <div className="relative overflow-hidden bg-gray-50 aspect-square">
          ...
          {/* Wishlist btn */}
          <button
            onClick={(e) => { e.preventDefault(); toggle(product); e.stopPropagation(); }}
            className={`absolute z-20 p-2 rounded-full backdrop-blur-sm transition-all opacity-0 group-hover:opacity-100 ${
              wished ? 'bg-red-500 text-white' : 'bg-white/80 text-gray-500 hover:bg-red-50 hover:text-red-500'
            }`}
            style={{ 
              top: product.badge && discount > 0 ? '4.5rem' : (product.badge || discount > 0 ? '3rem' : '0.75rem'),
              right: '0.75rem' 
            }}
          >
            <HeartIcon />
          ...
          {isAdmin && (
            <button
              ...
              className="absolute top-3 left-3 z-30 p-2 bg-slate-900 text-white rounded-full shadow-xl hover:bg-slate-800 transition-all group-hover:scale-110"
              style={{ left: product.badge ? '4rem' : '0.75rem' }}
          ...
          <div className="absolute bottom-3 left-3 right-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300 z-10">
            <div className="bg-white/95 backdrop-blur-sm text-gray-900 text-xs font-bold text-center py-2.5 rounded-xl shadow-lg uppercase tracking-tight">
              Quick View
            </div>
          </div>
        </div>
        ...
        <div className="p-3 flex flex-col flex-1">
          ...
          <h3 className="text-sm font-bold text-gray-900 leading-snug line-clamp-2 mb-2.5 group-hover:text-indigo-600 transition-colors">
            {product.name}
          </h3>
          ...
          {product.stock === 0 && <p className="text-[10px] text-red-500 font-semibold mt-1.5">Out of stock</p>}
          {product.stock > 0 && product.stock <= 5 && (
            <p className="text-[10px] text-amber-600 font-semibold mt-1.5">Only {product.stock} left!</p>
          )}
        </div>
      </Link>
    </motion.div>
  ...
  return (
    <div>
      <div className="bg-gradient-to-r from-indigo-700 via-indigo-600 to-purple-700">
        <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-12 py-6 sm:py-12">
          <nav className="flex items-center gap-1.5 text-[10px] text-indigo-200 mb-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            ...
            {hasFilters && (
              <button
                onClick={clearAll}
                className="inline-flex items-center gap-1 px-3 py-1.5 bg-white/10 backdrop-blur-sm border border-white/20 text-white text-[10px] font-semibold rounded-lg hover:bg-white/20 transition-all"
              >
                <XIcon /> Clear filters
              </button>
            )}
      ...
      <div className="mx-auto max-w-[1400px] px-4 sm:px-6 lg:px-12 py-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 w-full sm:w-auto hide-scrollbar">
            <button
              ...
              className={`shrink-0 px-4 py-2 text-xs font-bold rounded-full border-2 transition-all ${
            ...
            {categories.map((cat) => (
              <button
                ...
                className={`shrink-0 px-4 py-2 text-xs font-bold rounded-full border-2 transition-all flex items-center gap-1.5 ${
          ...
          <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto justify-end">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-2 bg-white border-2 border-gray-200 hover:border-indigo-300 rounded-xl text-[10px] font-bold text-gray-700 transition-all min-w-[130px] justify-between"
              >
                <span className="flex items-center gap-1.5">
                  <span>{currentSort.icon}</span> {currentSort.label}
                </span>
              ...
              <AnimatePresence>
                {sortOpen && (
                  <motion.div
                    ...
                    className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-2xl shadow-gray-200/80 border border-gray-100 overflow-hidden z-30"
                  >
                    {sortOptions.map((opt) => (
                      <button
                        key={opt.value}
                      ...
                      >
                        <span className="w-6 text-center text-base">{opt.icon}</span>
                        {opt.label}
                        {sortBy === opt.value && (
            ...
            <div className="flex bg-white border-2 border-gray-200 rounded-xl overflow-hidden">
              <button
                ...
                className={`p-2.5 transition-colors ${viewMode === 'grid' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-gray-600'}`}
              >
                <GridIcon />
              </button>
              <button
                onClick={() => setViewMode('list')}
                className={`p-2.5 transition-colors ${viewMode === 'list' ? 'bg-indigo-600 text-white' : 'text-gray-400 hover:text-gray-600'}`}
              >
                <ListIcon />
              </button>
            </div>
          ...
          <div className="flex flex-wrap items-center gap-2 mb-6">
            ...
            {searchQuery && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 text-xs font-bold rounded-full border border-indigo-100">
                🔍 "{searchQuery}"
                <button onClick={() => { const p = new URLSearchParams(searchParams); p.delete('q'); setSearchParams(p); }} className="hover:text-indigo-900">
                  <XIcon />
                </button>
              </span>
            )}
            {selectedCategory !== 'all' && (
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 text-xs font-bold rounded-full border border-indigo-100">
                ...
                <button onClick={() => setFilter('category', 'all')} className="hover:text-indigo-900">
                  <XIcon />
                </button>
              </span>
            )}
        ...
        <AnimatePresence mode="wait">
          {filteredProducts.length > 0 ? (
            ...
            >
              {filteredProducts.map((product) => (
                <ProductCard 
                  ...
                  viewMode={viewMode} 
                  isAdmin={isAdmin}
                  catName={categories.find((c) => c.id === product.category)?.name || product.category}
                />
              ))}
            </motion.div>
          ) : (
            ...
            >
              <div className="w-24 h-24 rounded-3xl bg-gray-100 flex items-center justify-center mx-auto mb-6 text-4xl">
                🔍
              </div>
              ...
              <button
                onClick={clearAll}
                className="px-8 py-3 bg-indigo-600 text-white text-sm font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-200"
              >
                Clear All Filters
              </button>
            </motion.div>
